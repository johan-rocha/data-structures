...
void em_ordem(no *raiz)
{
    ...
    while (current != NULL || !isEmpty(stack_top))
    {
        // Alcance o nó mais à esquerda da subárvore atual
        while (current != NULL)
        {
        ...
        // Current deve ser NULL neste ponto
        current = pop(&stack_top);
        printf("%d ", current->dado);

        // Começar a processar a subárvore direita
        current = current->dir;
    }
}
